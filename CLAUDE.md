...
- `client/public/assets/zones/` and `client/public/assets/props/` : generated background plates and midground prop sets (docs/ART-PIPELINE.md). App.jsx loads them into the renderer's BG_PLATES / PROP_SPRITES registries; missing files leave that scene's procedural art in place. Prop sets are slot arrays (0 = anchor, 1+ = small props) placed by a deterministic world-tile hash.
