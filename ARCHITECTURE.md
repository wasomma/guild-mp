...
Enforcement happens per intent before the simulation ever sees it. When OAuth is configured: intents targeting a Discord-owned character (skills, wardrobe, retelling their tale) require the socket to be bound to that exact snowflake; guild-wide intents (potion auto-toggles, legacy upgrades) require any authenticated user; characters not owned by a Discord identity (from open dev mode) remain manageable by any logged-in user; spectating stays open. Denied intents get an explicit denial message so the client can react. When OAuth is not configured, everything is open, which is the local development mode. Sessions survive server restarts and expire after 30 days; logout revokes them immediately.
