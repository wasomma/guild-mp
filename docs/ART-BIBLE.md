...
Status: **style locked 2026-07-21** from the owner's reference picks. **Character direction re-pivoted 2026-07-24:** the Phase 7D layered-puppet track paused after the owner judged the generated class bodies (tanks shipped v0.1.23, dps/healer rolls unshipped) **off-style against the kitsune** — gaunt photoreal faces in drab gray versus her soft painterly charm. The tank HD layers were rolled back (v0.1.25); only the kitsune full-body set remains HD. The new foundation is the **character creator** (v0.1.25): every hero is a procedural paperdoll composed from free identity catalogs — six races (Human, Elf, Kitsune-kin, Dwarf, Orc, Tiefling) with drawn features (ears, tail, build, beard, tusks, horns), nine skin tones (runtime ramp like hair tint), and starter undergarments. When HD generation resumes, it inherits the creator's race × gender taxonomy and must match the kitsune's painterly-soft look, not the photoreal mannequin lane — treat the kitsune as the style bar for every future character generation.
...
- Characters are **hybrid layered puppets** where HD layers exist (Phase 7D: generated hi-res base body per class × body type, gear and cosmetics as overlay layers — combinatorics preserved — animated by the engine's transform motion) and **procedural paperdolls** everywhere else. The gate is all-or-nothing per hero: one chunky procedural piece on a fine body reads worse than the classic look, so a missing layer keeps the whole hero procedural — see ART-PIPELINE.md Phase 7. Facings: east is combat canon (west mirrors); south is the feast's facing — the procedural feaster draws a true south face (centered symmetric features, south positions for race features and face marks) as of v0.1.26, and HD south layers are authored later against the same read; north waits for a scene that shows it. Every HD asset is named and registered facing-aware from day one.
