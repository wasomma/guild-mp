...
**Status: the HD kitsune SHIPPED in v0.1.22 (2026-07-22).** Equipping the **Kitsune Crown hairstyle + Nine-Tails cape** summons her: a source-native ~246px-art layered puppet (`assets/heroes/kitsune-e-body.png` from roll body-s2, plus one tail sprite drawn as five engine-swayed instances — see ART-PIPELINE Phase 7C). East-facing; south arrives with the feast facing in Phase F. The procedural cosmetic set below remains the catalog definition and the fallback look (other hairstyle/cape combinations still render the paperdoll, and the feast still uses procedural drawFeaster). Iterate with: `prototype/hd-preview.html` (2× canvas), classic via `qa-kitsune-preview.mjs` + `prototype/kitsune-preview.html`, regression via `qa-kitsune-set.mjs`.
